 ...
 * Which island has the "biggest bois" of Gentoo penguins?
...
```{r}    
  ...
  facet_wrap(~sex)
    ...
    ggplot(penguin_count, aes(x = reorder(species, percentage_p), 
                              y = percentage_p,
                              fill = species)) + 
      geom_bar(stat="identity", 
               position = "dodge")
...
```{r}
    penguins %>% 
     ...
     geom_point(aes(color = species), size = 5, alpha = .7) +# colouring each point by the species
      coord_flip()
...
```{r}
  ...
  ggplot(data, aes(fill = avg_hwy, area = avg_hwy, label = ... )) +
    geom_treemap() +
    geom_treemap_text(colour = "white",
                      place = "centre",
                      size = 5,
...
plot_2 <- plot +
  scale_x_continuous( # scale_x_continuous() for manipulating the x axis.
    ...
    name = "Proportion" # Renaming x axis to "Proportion"
  ) 
...
plot_4 <- plot_3 +
  geom_text(
    aes(label = paste0("  ", # setting a blank space at start of label
        ...
        color = prop > .03, # setting colour rule for prop values greater than .05
        hjust = prop > .03), # setting horizontal rule for prop values greater than .05
